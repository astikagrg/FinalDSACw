...
class SubsetFinder {
    ...
    SubsetFinder(int realArrLen, int[] a) {
        this.realArrLen = realArrLen; // store the real array length
        int len = 0; // store the length of the array

        int[] passer = new int[a.length]; // create a new array to store the array to be sorted

        // iterate through the array to be sorted
        for (int i = 0; i < a.length; i++) {
            boolean push = false; // initialize the push variable to false
            ...
            for (int j = 0; j < a.length; j++) {
                ...
                if (i >= j) {
                    if (i == a.length - 1) {
                        // if the index is eaual to the one less than length of the array change push to
                        // true
                        push = true;
                        break;
                    }
    ...
    void resultFinder() {
        ...
        if (primeItterator == realArrLen) {
            // if the prime iterator is greater than the element of the array print the
            // prime values
            ...
            for (int i = 0; i < realArrLen; i++) {
                // if the element is greater or equal to the prime iterator print the extra
                // values
                if (i >= primeItterator) {
